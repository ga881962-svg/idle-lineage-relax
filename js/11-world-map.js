...
function renderTownNPCs(townId) {
    renderTownNPCCards(townId);
}

// 🏘️ 所有安全區統一卡片式 NPC 導覽：名稱、職能、說明與圖示常駐，無須在場景中尋找小型角色。
function renderTownNPCCards(townId) {
    ...
    if (!container || !td) return;
    if (map) { map.classList.add('hidden'); map.classList.add('town-cards-mode'); }
    _townNpcSprites = [];
    container.innerHTML = '';
    container.classList.remove('hidden');
    container.classList.add('town-npc-cards');
    try { container.style.background = _townMapBg(townId) + ' center / cover no-repeat'; } catch (e) {}
    let vis = (td.npcs || []).filter(npc => {
        if (npc.id === 'npc_esti' || npc.id === 'npc_tros') return typeof clanNpcVisible === 'function' && clanNpcVisible(npc.id, townId);
        if (npc.darkOnly && player.cls !== 'dark') return false;
        if (npc.classicHide && player.classicMode) return false;
        if (npc.classicOnly && !player.classicMode) return false;
        return true;
    }).map(npc => ((npc.id === 'npc_esti' || npc.id === 'npc_tros') && typeof clanNpcDisplayName === 'function') ? Object.assign({}, npc, { n: clanNpcDisplayName() }) : npc);
    if (townId === 'town_pride') vis.push({ id: '_pride_entrance', n: '傲慢之塔', title: '入口', d: '挑戰傲慢之塔，查看攀登與排名紀錄。', _float: 'pride' });
    if (townId === 'town_rift') vis.push({ id: '_rift_entrance', n: '時空裂痕', title: '入口', d: '進入時空裂痕，展開特殊挑戰。', _float: 'rift' });
    try { if (typeof getWanderingBuyersForTown === 'function') { let ws = getWanderingBuyersForTown(townId); if (Array.isArray(ws)) vis.push.apply(vis, ws); } } catch (e) {}
    let title = (typeof mapDisplayName === 'function' && mapDisplayName(townId)) || td.n || '安全區域';
    let intro = document.createElement('div');
    intro.className = 'town-card-intro';
    intro.innerHTML = `<div class="town-card-town-name">${title}</div><div>這裡是安全區域，您的體力與魔力已完全恢復。</div>`;
    container.appendChild(intro);
    let iconFor = npc => {
        let s = ((npc.title || '') + ' ' + (npc.type || '') + ' ' + (npc.id || '')).toLowerCase();
        if (/倉|warehouse/.test(s)) return '🗄️'; if (/商|shop|merchant/.test(s)) return '💰';
        if (/製作|craft|鍛/.test(s)) return '⚒️'; if (/拍賣|auction/.test(s)) return '🗓️';
        if (/傳送|港|入口|rift/.test(s)) return '⛵'; if (/血盟|clan|公會/.test(s)) return '⚔️';
        if (/試煉|quest/.test(s)) return '📜'; if (/領取|贊助|gift/.test(s)) return '💎';
        return '👤';
    };
    vis.forEach(npc => {
        let card = document.createElement('button');
        card.type = 'button'; card.className = 'town-npc-card'; card.dataset.npcId = npc.id || '';
        let role = npc.title || (npc._wanderer ? '玩家收購' : 'NPC');
        let desc = npc.d || `與${npc.n}交談，使用${role}功能。`;
        card.innerHTML = `<span class="town-npc-card-copy"><span class="town-npc-card-name">${npc.n}</span><span class="town-npc-card-role">[${role}]</span><span class="town-npc-card-desc">${desc}</span></span><span class="town-npc-card-icon" aria-hidden="true">${iconFor(npc)}</span>`;
        if (npc._wanderer) card.onclick = () => openWanderingBuyerDialog(npc.id);
        else if (npc._float === 'pride') card.onclick = () => openTownFloatWindow('傲慢之塔', '排名挑戰', renderPrideEntrance);
        else if (npc._float === 'rift') card.onclick = () => openTownFloatWindow('時空裂痕', '進入', renderRiftEntrance);
        else card.onclick = () => interactNPC(npc.id, townId);
...
function renderArkataBuyback(el) {
        ...
        return `
            <div class="flex items-center justify-between gap-2 bg-slate-800/60 border border-slate-600 rounded p-3">
                <div class="text-sm leading-relaxed"><span class="${cl} font-bold">${nm}</span>${when ? `<span class="text-xs text-slate-500">（${when} 遺失）</span>` : ''}<br>
                    <span class="text-xs text-slate-400">贖回費用 <span class="text-sky-300 font-bold">${ARKATA_REDEEM_COST.toLocaleString()}</span> 贊助鑽石</span></div>
    ...
    el.innerHTML = `
        <div class="flex flex-col gap-3 p-1">
            ...
            <div class="text-xs text-slate-400">邪惡狀態下死亡遺失的裝備會被記錄：${lost.length} / 5（滿 5 件後新的遺失會擠掉最舊的一筆）・持有贊助鑽石：<span class="text-sky-300">${dia.toLocaleString()}</span></div>
...
function arkataRedeemItem(i) {
    ...
    if (typeof pandoraGetSharedDiamonds !== 'function' || typeof pandoraAdjustSharedDiamonds !== 'function') { logSys('<span class="text-red-400">贊助鑽石系統尚未載入，請重新整理後再試。</span>'); return; }
    if (pandoraGetSharedDiamonds() < ARKATA_REDEEM_COST) { logSys(`<span class="text-red-400">贊助鑽石不足（需要 ${ARKATA_REDEEM_COST.toLocaleString()}）。</span>`); return; }
    ...
    logSys(`<span class="text-sky-300">聖使阿卡塔以聖光為你尋回了 <span class="font-bold">${nm}</span>（花費 ${ARKATA_REDEEM_COST.toLocaleString()} 贊助鑽石）。</span>`);
...
function interactNPC(npcId, townId) {
    ...
    } else if (npc.type === 'sponsor') {
        renderSponsorMerchant(contentDiv);
...
// 手機瀏覽器有時會把浮動視窗內的 onclick 事件交給底層地圖；在捕獲階段直接處理關閉鍵，
// 讓觸控與滑鼠都一定能關閉 NPC 視窗。
document.addEventListener('click', function(event) {
    const closeButton = event.target && event.target.closest ? event.target.closest('#town-interaction-close') : null;
...
function _npcFrames(key) {
    ...
    if (cat) for (let i = 0; i < cat.f; i++) { let im = new Image(); im.src = assetUrl('assets/npc/' + cat.g + '/idle_' + i + '.png'); arr.push(im); }
...
function _npcWeaponFrames(key) {   // 🔥 火焰/武器疊層幀(idle_w_N)：僅 NPC_SPR 有 w 的（如宙斯之熔岩高崙）·與本體同幀數同步
    ...
    let cat = NPC_SPR[key], arr = [];
    if (cat && cat.w) for (let i = 0; i < cat.w; i++) { let im = new Image(); im.src = assetUrl('assets/npc/' + cat.g + '/idle_w_' + i + '.png'); arr.push(im); }
...
function _townMapBg(townId) {
    ...
    try {
        let localFallback = (typeof encodedAreaBg === 'function') ? encodedAreaBg('村莊周邊') : assetUrl('assets/area/村莊周邊.jpg');
        if (typeof TOWN_BG_1920 !== 'undefined' && TOWN_BG_1920[townId] && typeof areaBg1920 === 'function')
            return ov + ', url("' + areaBg1920(TOWN_BG_1920[townId]) + '"), url("' + localFallback + '")';
        let timg = ((typeof SPECIAL_TOWN_BG !== 'undefined') && SPECIAL_TOWN_BG[townId])
            || ((typeof TOWN_AREA_BG !== 'undefined') && cat && TOWN_AREA_BG[cat]) || null;
        if (timg) return ov + ', url("' + assetUrl('assets/background/' + timg) + '"), url("' + localFallback + '")';
    } catch (e) {}
...
function _townCastleCrownHtml(npc) {
    ...
    return (royalNpc || royalPlayer)
        ? '<img class="tn-castle-crown" src="' + assetUrl('assets/ui/castle-crown.gif?v=v3.6.22') + '" alt="" aria-hidden="true" draggable="false">'
        : '';
}
...
function renderTownNPCMap(townId) {
    ...
    map.innerHTML = '';
    _townNpcSprites = [];
    // 直接指定完整 background，而非只寫 backgroundImage：小視窗／既有樣式中的 background shorthand
    // 不會再把城鎮圖片覆蓋成單色底。
    try { map.style.background = _townMapBg(townId) + ' center / cover no-repeat'; } catch (e) {}
    let td = DB.towns[townId];
    if (!td) return;
    // 與舊卡片清單相同的可見性過濾
    let vis = (td.npcs || []).filter(npc => {
    ...
    vis.forEach((npc, i) => {
        ...
        el.innerHTML =
            ...
            crownHtml +
            '<img class="tn-shadow" src="' + assetUrl('assets/npc/' + cat.g + '/idle_s_0.png') + '" alt="" onload="this.parentElement.classList.add(\'has-tn-shadow\')" onerror="this.remove()">' +   // 🌑 v3.3.5 真實影子 sprite(body gfx+1·共畫布疊本體對齊)；有影子→onload 標記父層隱藏後備橢圓；無影子(職業動畫/老 gfx/告示)→404 remove→改用 CSS 橢圓後備影子(v3.3.18)
            '<img class="tn-body"' + (cat.tint ? (' style="filter:' + cat.tint + '"') : '') + ' src="' + assetUrl('assets/npc/' + cat.g + '/idle_0.png') + '" alt="">' +
            (cat.w ? '<img class="tn-weapon" src="' + assetUrl('assets/npc/' + cat.g + '/idle_w_0.png') + '" alt="" onerror="this.remove()">' : '');   // 🔥 火焰/武器疊層(screen 混合·宙斯之熔岩高崙的燃燒特效)
