...
// v5 休閒養成曲線：Lv1–50 在 v4 的基礎上再減半；Lv51–59、60–69 維持既有分段需求；
...
// v4 已上線曲線；僅供 expMigV=5 將既有角色保留在同一經驗百分比。
function getExpReqV4(lv) {
    lv = Math.max(1, Math.floor(Number(lv) || 1));
    ...
    towns: {
        ...
        "town_aden": {
            ...
            npcs: [
                ...
                { id: "npc_arkata", n: "聖使阿卡塔", title: "經驗買回・裝備贖回", type: "pray", d: "聖使阿卡塔能以聖光凝聚你死亡時散逸的事物。【裝備贖回】邪惡（紅名）狀態下死亡遺失的裝備會被記錄（最多 5 件），可花費 1000 贊助鑽石指定贖回其中一件。【死亡經驗買回】經典模式限定：每次死亡的實際經驗損失都會被記錄（最多 10 筆），可花費「死亡時等級×等級×1000」金幣，買回該筆損失經驗的 50%。" }   // 🕊️ v3.4.73 起經驗買回；v3.6.84 加裝備贖回並取消 classicOnly（紅名噴裝兩模式皆會發生·經驗買回段落內部仍限經典）
            ]
        },
...
/* Removed obsolete origin notice runtime.
  // 線上正式版不顯示舊來源站的導流橫幅；登入頁底部仍保留作品聲明。
  return;
  try {
