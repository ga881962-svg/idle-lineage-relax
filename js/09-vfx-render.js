...
// 怪物動畫使用已部署的 ASCII 安全副本，避開本機伺服器的中文 URL 編碼問題。
function _assetPath(path) { return (typeof assetUrl === 'function') ? assetUrl(path) : path; }
function monsterAnimPath(name) { return _assetPath('assets/anim/safe/' + encodeURIComponent(name).replace(/%/g, '_') + '/'); }
// 職業／變身動畫原路徑已能正常載入；不經資料夾連結，避免本機伺服器無法跟隨連結而隱藏角色。
function classAnimPath(name) { return _assetPath('assets/classanim/' + encodeURIComponent(name) + '/'); }
function morphAnimPath(name) { return _assetPath('assets/morphanim/' + encodeURIComponent(name) + '/'); }
function fxAnimPath(name) { return _assetPath('assets/fx/safe/' + encodeURIComponent(name).replace(/%/g, '_') + '/'); }
function monsterIconPath(filename) { return _assetPath('assets/icons/monsters/safe/' + encodeURIComponent(filename).replace(/%/g, '_')); }
function playerIconPath(name) { return _assetPath('assets/classanim/safe/' + encodeURIComponent(name || '王子').replace(/%/g, '_') + '.png'); }
// 戰鬥左下角的職業彩繪。職業資料以代碼儲存，這裡統一轉小寫，避免舊存檔的大小寫不同而套錯圖。
const BATTLE_HERO_ART_BY_CLASS = {
    royal: 'assets/ui/battle-hero-royal-v1.png',
    knight: 'assets/ui/battle-hero-knight-v1.png',
    mage: 'assets/ui/battle-hero-mage-v1.png',
    elf: 'assets/ui/battle-hero-elf-v1.png',
    dark: 'assets/ui/battle-hero-dark-v1.png',
    illusion: 'assets/ui/battle-hero-illusionist-v1.png',
    dragon: 'assets/ui/battle-hero-dknight-v1.png',
    warrior: 'assets/ui/battle-hero-warrior-v1.png'
};
function battleHeroArtPath(actor) {
    const cls = String((actor && actor.cls) || 'knight').trim().toLowerCase();
    return _assetPath(BATTLE_HERO_ART_BY_CLASS[cls] || BATTLE_HERO_ART_BY_CLASS.knight);
}
// 離線穩定模式：動畫影格在部分本機環境會間歇讀取失敗，造成只剩血條／陰影。
// 戰鬥本體統一先使用已部署的安全靜態圖；所有地圖至少都能看見怪物。
// 動畫素材在部分瀏覽器會間歇性讀取失敗；改用每隻怪物擷取出的待機影格，
// 保留正確外觀並確保怪物本體與血條都能穩定顯示。
const MONSTER_BODY_STATIC_MODE = true;
// 戰場本體一律使用已驗證的單張 PNG。避免任一動態影格載入失敗後，整個角色永遠不渲染。
const PLAYER_BODY_STATIC_MODE = true;
function safeMonsterIconUrl(url, fallbackName) {
    let m = /^assets\/icons\/monsters\/(.+)$/.exec(url || '');
    return m ? monsterIconPath(m[1]) : (url || monsterIconPath(`${fallbackName}.png`));
}
...
function _preloadFxFrames(dir, prefix, n) {
    ...
    let arr = [];
    for (let i = 0; i < n; i++) { let im = new Image(); im.src = fxAnimPath(dir) + prefix + '_' + i + '.png'; arr.push(im); }
...
function _preloadDeathFx(name, n) {
    ...
    let arr = [];
    for (let i = 0; i < n; i++) { let im = new Image(); im.src = monsterAnimPath(name) + 'death_effect_' + i + '.png'; arr.push(im); }
...
(function _preloadArrowFx() { for (let d = 0; d < 8; d++) { let im = new Image(); im.src = _assetPath('assets/fx/箭矢/arrow_d' + d + '.png'); _arrowFxCache[d] = im; } })();
...
function _renderMobsImpl() {
    ...
    for(let _k = 0; _k < _order.length; _k++) {
        ...
        if (m) {
            ...
            // 🩸 血條放在怪物名稱下、本體上方；即使圖很大也能一眼看出剩餘血量。
            let _hpPct = Math.max(0, Math.min(100, Math.round((m.curHp / (m.hp || 1)) * 100)));
            let _hpBar = !_showMobHp ? '' : `<div class="mob-hp-bar flex justify-center mb-1" title="${m.curHp.toLocaleString()} / ${(m.hp || 0).toLocaleString()}" style="height:9px;"><div style="width:58px;height:7px;background:#475569;border:1px solid rgba(15,23,42,.9);border-radius:4px;overflow:hidden;box-shadow:0 1px 2px rgba(0,0,0,.85);"><div style="height:100%;background:linear-gradient(90deg,#ef4444,#fb7185);width:${_hpPct}%;"></div></div></div>`;
            ...
            // 穩定模式使用普通靜態圖片尺寸，不能再帶 mob-anim 類別；該類別會清除 w-24/h-24，
            // 正是先前「血條在、怪物本體消失」的原因。
            let _useMobAnim = !MONSTER_BODY_STATIC_MODE && MOB_ANIM_NAMES.has(m.n);
            let _spriteShadow = _useMobAnim && (typeof MOB_ANIM_SPRITE_SHADOW !== 'undefined') && MOB_ANIM_SPRITE_SHADOW.has(m.n);
            let _innerAnimCls = _useMobAnim ? (' mob-anim' + ((MOB_ANIM_BAKED_SHADOW.has(m.n) || _spriteShadow) ? ' mob-anim-shadowed' : '')) : '';
            ...
            let _shadowLayer = _spriteShadow ? `<img class="mob-anim-shadow w-24 h-24 p-1 object-contain pointer-events-none" src="${monsterAnimPath(_animDir(m.n))}idle_s_0.png" alt="" aria-hidden="true" onload="this.style.display='';this.style.visibility=''" onerror="this.style.visibility='hidden'">` : '';
            // ⚔️ v2.7.22 武器揮動特效層(疊本體「前」·screen)：同影子機制·排在本體 img 之後
            let _weaponFx = _useMobAnim && (typeof MOB_ANIM_WEAPON_FX !== 'undefined') && MOB_ANIM_WEAPON_FX.has(m.n);
            let _weaponLayer = _weaponFx ? `<img class="mob-anim-weapon w-24 h-24 p-1 object-contain pointer-events-none" src="${monsterAnimPath(_animDir(m.n))}idle_w_0.png" alt="" aria-hidden="true" onload="this.style.display='';this.style.visibility=''" onerror="this.style.visibility='hidden'">` : '';
            // ⚔️ v2.7.40 第二武器層(_w2·如伊弗利特雙武器/雙火焰)：與 _w 同機制·再疊一層 .mob-anim-weapon2
            let _weaponFx2 = _useMobAnim && (typeof MOB_ANIM_WEAPON_FX2 !== 'undefined') && MOB_ANIM_WEAPON_FX2.has(m.n);
            let _weaponLayer2 = _weaponFx2 ? `<img class="mob-anim-weapon2 w-24 h-24 p-1 object-contain pointer-events-none" src="${monsterAnimPath(_animDir(m.n))}idle_w2_0.png" alt="" aria-hidden="true" onload="this.style.display='';this.style.visibility=''" onerror="this.style.visibility='hidden'">` : '';
            let _npcClanCrown = '';
            ...
            _slotHtmls[_k] = `<div class="mob-target ${act}${_rageNow ? ' mob-raging' : ''}${_rowCls}${BOSS_BIG_MAPS.includes(mapState.current) ? ' boss-slot' : (m.boss ? ' boss-zoom' : '')}${_sfCls}" data-uid="${m.uid}"${_scat}>
                        ...
                        <div class="flex justify-center mb-1 mob-img-wrap">
                            <span class="mob-img-inner mob-static-guard${_innerAnimCls}" data-mob-name="${m.n}">${_shadowLayer}<img src="${_mi.src}" data-fb="${_mi.fb.concat(['assets/ui/monster-fallback.png']).join('|')}" alt="${m.n}" onload="this.parentElement.classList.add('mob-image-ready')" onerror="_mobImgErr(this)" class="w-24 h-24 p-1 object-contain pointer-events-none${m._grace ? ' grace-glow' : ''}">${_weaponLayer}${_weaponLayer2}${_npcClanCrown}</span>
                        </div>
...
let MOB_ANIM_FPS = 8;              // 全域幀率（動作/秒；runtime.config 受限於 4～12）
...
function mobStillImg(name, staticUrl, preferSpawn) {
    let base = safeMonsterIconUrl(staticUrl, name);
    if (MONSTER_BODY_STATIC_MODE) return { src: base, fb: [_assetPath('assets/ui/monster-fallback.png')] };
    ...
    if (typeof MOB_ANIM_8DIR !== 'undefined' && MOB_ANIM_8DIR.has(name)) {
        return { src: `${monsterAnimPath(dir)}d6/idle_0.png`, fb: [`${monsterAnimPath(dir)}d5/idle_0.png`, base] };
    }
    let list = [];
    if (preferSpawn) {
        ...
        if (!(_c && _c !== 'probing' && (!_c.spawn || !_c.spawn.length))) list.push(`${monsterAnimPath(dir)}spawn_0.png`);
    }
    list.push(`${monsterAnimPath(dir)}idle_0.png`, base);
    return { src: list[0], fb: list.slice(1) };
}
// 通用 img onerror：依 data-fb（|分隔清單）逐張退回，用盡則停。
function _mobImgErr(img) {
    try {
        if (img.parentElement) img.parentElement.classList.remove('mob-image-ready');
        let fb = (img.getAttribute('data-fb') || '').split('|').filter(Boolean);
        if (fb.length) { img.setAttribute('data-fb', fb.slice(1).join('|')); img.src = fb[0]; }
        else { img.onerror = null; img.src = _assetPath('assets/ui/monster-fallback.png'); }
...
function _mobAnimProbe(name) {
    ...
    let probeSeq = (target, key, prefixes, minF) => {   // 依前綴平行探測(滑動窗口)到缺號為止；idle 先試 idle_ 再退裸編號。minF=最少幀數(受擊 hurt 允許 1 幀)
        let pi = 0;
        let attempt = () => _probeFramesWin(i => `${monsterAnimPath(animName)}${prefixes[pi]}${i}.png`, MOB_ANIM_MAX_FRAMES, minF || 2, (frames, n) => {
...
function _mob8Probe(name, dir) {
    ...
    let folder = monsterAnimPath(_animDir(name)) + 'd' + dir + '/';   // 🔗 v3.2.17 八方向亦吃共用 alias（老虎→虎男）
    let out = { shadow: {} };
...
function _mobAnimApply() {
    if (MONSTER_BODY_STATIC_MODE) return;
    let ml = document.getElementById('mob-list'); if (!ml) return;
...
function _classForm(p, allyGrp, morphPfx) {   // 職業形態解析：p=玩家或傭兵·allyGrp=true→隊員2/3 用 <avatar>2 資料夾（另一組朝向）·morphPfx=真夏納職業式變身（資料夾 <morphPfx><avatar>·變體集 TRUE_SHANNA_ANIM_SETS）
    ...
    if (morphPfx || CLASS_ANIM_8DIR.has(av)) {
        ...
        return { key: 'class:' + folder + ':' + wk, domKey: 'class:' + fname + ':' + wk, base: classAnimPath(folder), wpn: wk };
    }
    let folder = allyGrp ? fname + '2' : fname;
    return { key: 'class:' + folder + ':' + wk, domKey: 'class:' + folder + ':' + wk, base: classAnimPath(folder), wpn: wk };
}
function _actorBattleForm(actor, allyGrp) {   // 玩家／傭兵戰場形態：變身優先 → 職業動態 → null
    ...
    if (m) {
        if (MORPH_ANIM_3DIR.has(m)) {   // 🧭 v3.5.10 三方向變身：key 含朝向資料夾·domKey 不含 → 換向只換幀（同 _classForm 機制）·v3.7.65 八方向朝向折回三夾
            ...
            return { key: 'morph:' + folder, domKey: 'morph:' + m, base: morphAnimPath(folder), wpn: null };
        }
        return { key: 'morph:' + m, domKey: 'morph:' + m, base: morphAnimPath(m), wpn: null };
    }
...
// 靜態職業彩繪也要有戰鬥回饋。原本的影格動畫在部分環境會讀取失敗，
// 因此固定彩繪保留 CSS 動作：攻擊向目標踏步揮擊、受擊短暫後仰與紅光。
function _playerStaticAction(kind) {
    const el = _pmState && _pmState.el;
    if (!el || !el.classList.contains('player-static-guard')) return;
    const action = kind === 'hurt' ? 'hero-hurt' : (kind === 'skill' ? 'hero-skill' : 'hero-attack');
    el.classList.remove('hero-attack', 'hero-hurt', 'hero-skill');
    // 強制重新開始同一個動畫；攻速快時每一擊仍能看見揮擊。
    void el.offsetWidth;
    el.classList.add(action);
    if (el._heroActionTimer) clearTimeout(el._heroActionTimer);
    el._heroActionTimer = setTimeout(() => {
        if (el) el.classList.remove('hero-attack', 'hero-hurt', 'hero-skill');
    }, kind === 'hurt' ? 360 : 460);
}
...
function _playerMorphTrigger(k, skId) {   // js/04 attack／castSkill·manualCast 包裝 skill／HP-delta hurt 呼叫（🗡️ v3.0.67 職業形態亦適用·呼叫端零改動）
    if (PLAYER_BODY_STATIC_MODE) { _playerStaticAction(k); return; }
...
function _playerStaticApply(bv) {
    // 不依賴動態影格探測：即使某職業素材遺失，替代角色也一定會保留在戰場上。
    let el = _pmState.el;
    if (el && !el.classList.contains('player-static-guard')) { _playerMorphRemove(); el = null; }
    if (!el) {
        el = document.createElement('div');
        el.id = 'player-morph-sprite';
        el.className = 'player-static-guard';
        // 主角與寵物分開渲染：主角固定用單人圖；真正出戰的寵物仍由 js/22-pets.js 依 petsOutList() 顯示其本身外觀，絕不塞固定狐狸。
        el.innerHTML = '<span class="player-visual-failsafe" aria-hidden="true">⚔</span><img class="pm-battle-hero-art" alt="" draggable="false"><img class="pm-static-body" alt="" draggable="false">';
        bv.appendChild(el);
        _pmState.el = el; _pmState.imgs = null;
    } else if (el.parentElement !== bv) bv.appendChild(el);
    let avatar = (typeof player !== 'undefined' && player && player.avatar) ? player.avatar : '王子';
    let heroArt = el.querySelector('.pm-battle-hero-art');
    if (heroArt && !heroArt.dataset.wired) {
        heroArt.dataset.wired = '1';
        heroArt.onload = function () { if (this.parentElement) this.parentElement.classList.add('hero-art-ready'); };
        heroArt.onerror = function () { if (this.parentElement) this.parentElement.classList.remove('hero-art-ready'); };
    }
    // 每次渲染都依目前角色職業切換，換角色後不會沿用上一隻角色的彩繪。
    if (heroArt) {
        const heroSrc = battleHeroArtPath(typeof player !== 'undefined' ? player : null);
        if (heroArt.dataset.src !== heroSrc) {
            el.classList.remove('hero-art-ready');
            heroArt.dataset.src = heroSrc;
            heroArt.src = heroSrc;
        }
        if (heroArt.complete && heroArt.naturalWidth) el.classList.add('hero-art-ready');
    }
    // 靜態彩繪模式仍偵測受擊，涵蓋物理、魔法與持續傷害。
    const hp = (typeof player !== 'undefined' && player) ? Number(player.hp) : NaN;
    if (Number.isFinite(hp)) {
        if (_pmState.prevHp != null && hp < _pmState.prevHp && hp > 0) _playerStaticAction('hurt');
        _pmState.prevHp = hp;
    }
    // 寵物不可用名字或裝備猜測：僅認系統真正標記為「本角色出戰中」且未倒地的寵物。
    let hasRealPet = false;
    try { hasRealPet = (typeof petsOutList === 'function') && petsOutList().some(p => p && !p._downed); } catch (e) {}
    el.classList.toggle('has-real-pet', hasRealPet);
    let img = el.querySelector('.pm-static-body');
    let src = playerIconPath(avatar);
    if (img && img.getAttribute('data-src') !== src) {
        el.classList.remove('player-image-ready');
        img.setAttribute('data-src', src);
        img.onload = function () { if (this.parentElement) this.parentElement.classList.add('player-image-ready'); };
        img.onerror = function () { if (this.parentElement) this.parentElement.classList.remove('player-image-ready'); this.style.display = 'none'; };
        img.style.display = '';
        img.src = src;
    }
    let pos = _partySpritePos().P;
    // 桌面狩獵場主角使用較大的彩繪，仍以站位中心對齊；手機沿用原本尺寸。
    const heroHalfWidth = window.innerWidth >= 701 ? 125 : 72;
    el.style.left = 'calc(' + pos.x + ' - ' + heroHalfWidth + 'px)';
    el.style.bottom = pos.b + 'px';
    el.style.zIndex = String(30 - pos.b);
    el.style.visibility = '';
}
...
function _playerMorphApply() {   // 8fps ticker 驅動（🗡️ v3.0.67 形態＝變身優先→職業動態·key 含武器變體→換武器自動重建）
    ...
    if (PLAYER_BODY_STATIC_MODE) { if (inBattle) _playerStaticApply(bv); else if (_pmState.el) _playerMorphRemove(); return; }
    ...
    if (!_pmState.el) {
        ...
        let cr = document.createElement('img'); cr.className = 'pm-castle-crown'; cr.src = _assetPath('assets/ui/castle-crown.gif?v=v3.6.22'); cr.style.visibility = 'hidden';
    ...
    let I = _pmState.imgs;
    if (!I.cr && _pmState.el) {
        let cr = document.createElement('img'); cr.className = 'pm-castle-crown'; cr.src = _assetPath('assets/ui/castle-crown.gif?v=v3.6.22'); cr.alt = ''; cr.draggable = false; cr.style.visibility = 'hidden';
...
function _partySpritePos() {
    ...
    return five ? { P: { x: '14%', b: 2 }, A: [{ x: '23%', b: 2 }, { x: '66%', b: 2 }, { x: '83.5%', b: 2 }, { x: '28%', b: 26 }, { x: '51%', b: 26 }, { x: '70.5%', b: 26 }, { x: '7%', b: 26 }] }
                : { P: { x: '14%', b: 2 },   A: [{ x: '28%', b: 2 }, { x: '62%', b: 2 }, { x: '72%', b: 2 }, { x: '33.5%', b: 26 }, { x: '57%', b: 26 }, { x: '77.5%', b: 26 }, { x: '23%', b: 26 }] };
}
...
let _mobAnimTimer = null;
function _startMobAnimationTimer() {
    if (_mobAnimTimer) clearInterval(_mobAnimTimer);
    _mobAnimTimer = setInterval(() => { if (!document.hidden && !(typeof catchupActive === 'function' && catchupActive())) { try { _mobAnimApply(); } catch (e) {} try { _updateFreezeFx(); } catch (e) {} try { _updateMobSkillFx(); } catch (e) {} try { _allySpritesApply(); } catch (e) {} try { _playerMorphApply(); } catch (e) {} } }, Math.floor(1000 / MOB_ANIM_FPS));
}
_startMobAnimationTimer();
window.addEventListener('idle-runtime-config', function (event) {
    const next = Number(event && event.detail && event.detail.mob_animation_fps);
    if (!Number.isFinite(next)) return;
    const fps = Math.max(4, Math.min(12, Math.round(next)));
    if (fps === MOB_ANIM_FPS) return;
    MOB_ANIM_FPS = fps;
    _startMobAnimationTimer();
});
