...
// 野外的假玩家、嗆聲追殺與血盟 NPC 戰鬥已停用；此模板仍供競技場使用。
const WILD_PLAYER_NPC_ENABLED = false;
...
function pvpEnsureState() {
    ...
    // 清理舊版本留下的假玩家追殺資料，避免換圖後又生成「白目玩家」。
    if (!WILD_PLAYER_NPC_ENABLED) {
        player.pvpOn = false;
        player.trollPlayers = [];
        player.pvpRevengeList = [];
        try {
            if (typeof mapState !== 'undefined' && mapState && Array.isArray(mapState.mobs)) {
                mapState.mobs = mapState.mobs.filter(m => m && !m.trollPlayer);
                if (mapState.targetIdx >= mapState.mobs.length) mapState.targetIdx = 0;
            }
        } catch (e) {}
    }
...
function pvpMarkForChase(entry) {
    if (!WILD_PLAYER_NPC_ENABLED) return;
...
function spawnMob(idx) {
    ...
    let npcClanBattle = WILD_PLAYER_NPC_ENABLED && !_antharasDungeon && typeof npcClanGroupBattleActive === 'function' && npcClanGroupBattleActive();
    let wcMassTauntBattle = WILD_PLAYER_NPC_ENABLED && !_antharasDungeon && typeof wcMassTauntGroupBattleActive === 'function' && wcMassTauntGroupBattleActive();
    if (!WILD_PLAYER_NPC_ENABLED) delete mapState._trollSpawn;
    ...
    if(!wantBoss) {
        ...
        if (WILD_PLAYER_NPC_ENABLED && siegeArea) {
        ...
        let _wildPvpChance = _clanEncounter && _clanEncounter.active
            ...
            : PVP_WILD_CHANCE;
        let _wildPvpAllowed = WILD_PLAYER_NPC_ENABLED && (!!player.pvpOn || !!(_clanEncounter && _clanEncounter.npcInitiated));
        ...
        if (WILD_PLAYER_NPC_ENABLED && !_antharasDungeon && player.trollPlayers && player.trollPlayers.length) {
            let _now = Date.now();
    ...
    if (!WILD_PLAYER_NPC_ENABLED) {
        // 舊版血盟／世界頻道事件可能殘留這些旗標，一律不再把它們轉成假玩家。
        wcMassTauntBattle = false;
        npcClanBattle = false;
        delete mapState._trollSpawn;
    }
    let _actualPlayerEncounter = WILD_PLAYER_NPC_ENABLED && mapState._trollSpawn && DB.mobs[mobId] && DB.mobs[mobId].trollPlayer;
    if (WILD_PLAYER_NPC_ENABLED && !npcClanBattle && !wcMassTauntBattle && _actualPlayerEncounter && typeof wcMassTauntMaybeStartGroupBattle === 'function' &&
        wcMassTauntMaybeStartGroupBattle(mapState._trollSpawn)) wcMassTauntBattle = true;
    if (WILD_PLAYER_NPC_ENABLED && wcMassTauntBattle && typeof wcMassTauntGroupBattleNextOpponent === 'function') {
    ...
    } else if (WILD_PLAYER_NPC_ENABLED && npcClanBattle && typeof npcClanCreateGroupBattleOpponent === 'function') {
    ...
    if(base.trollPlayer) {   // 😤 白目玩家：等級縮放＋名稱=叫賣NPC本人＋戰鬥動態=玩家職業動畫（v3.5.64·assets/anim/玩家<avatar>·從職業動畫產出·idle/attack/hurt/death/skill＋_s 影子全套·動態註冊 MOB_ANIM_ALIAS 真共用）
        ...
        const _pvpAnimBase = (typeof classAnimPath === 'function')
            ? classAnimPath((mapState.mobs[idx]._pvpAvatar || "男戰士") + "F")
            : ((typeof assetUrl === 'function')
                ? assetUrl("assets/classanim/" + (mapState.mobs[idx]._pvpAvatar || "男戰士") + "F/")
                : ("assets/classanim/" + (mapState.mobs[idx]._pvpAvatar || "男戰士") + "F/"));
        mapState.mobs[idx].img = _pvpAnimBase + "unarmed_idle_0.png";   // 動畫缺檔時的靜態後備
