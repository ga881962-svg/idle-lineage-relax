...
(function () {
    ...
    // 離線死亡後的「回原練功點」規則：只要該地圖的再次進入會消耗
    // 鑰匙、封印或傳送入場道具，就絕不自動回去，避免離線繞過入場成本。
    // 這裡直接讀取地圖既有的入場設定，而不是另外維護一份地圖清單。
    const OFFLINE_AUTO_REVIVE_DELAY_MS = 30000;
    const OFFLINE_AUTO_REVIVE_LIMIT = 12;
    const OFFLINE_RETURN_ENTRY_COSTS = {
        dark_elf_sanctuary: 'item_dk_book',
        cursed_dark_elf_sanctuary: 'item_dk_book',
        collapsed_elder_council_hall: 'item_giltas_seal'
    };

    function _offlineReturnBlocker(map) {
        map = String(map || '');
        let costId = _offlineBossRoomCostId(map) || OFFLINE_RETURN_ENTRY_COSTS[map] || '';
        if (!costId) return null;
        let item = typeof DB !== 'undefined' && DB.items ? DB.items[costId] : null;
        return { costId: costId, name: item && item.n ? item.n : '入場道具' };
    }

    function _offlineAutoRevivePlan(profile, requestedElapsed, efficiency) {
        let blocker = _offlineReturnBlocker(profile && profile.map);
        let remaining = Math.max(0, Math.floor(_offlineFinite(requestedElapsed, 0)));
        let elapsed = 0, combatMs = 0, reviveCount = 0;
        let potionUsed = 0, potionBought = 0, potionCost = 0, potionId = '';
        let firstDeathAtMs = 0, finalDied = false;

        while (remaining > 0) {
            let part = _offlineSurvivalPlan(profile, remaining, efficiency, true);
            elapsed += part.elapsedMs;
            combatMs += part.combatMs;
            potionUsed += part.potionUsed || 0;
            potionBought += part.potionBought || 0;
            potionCost += part.potionCost || 0;
            potionId = part.potionId || potionId;
            if (!part.died) break;
            if (!firstDeathAtMs) firstDeathAtMs = elapsed;

            // 有入場成本的地圖：死亡後不能免費回原地，沿用原本的死亡結算。
            if (blocker) { finalDied = true; break; }

            reviveCount += 1;
            // 一般狩獵圖在原地復活；30 秒復活時間不計入收益。
            remaining = Math.max(0, remaining - part.elapsedMs - OFFLINE_AUTO_REVIVE_DELAY_MS);
            if (player) {
                player.dead = false;
                player.hp = Math.max(1, Number(player.mhp) || Number(player.hp) || 1);
                player.mp = Math.max(0, Number(player.mmp) || Number(player.mp) || 0);
            }
            if (reviveCount >= OFFLINE_AUTO_REVIVE_LIMIT && remaining > 0) {
                finalDied = true;
                break;
            }
        }
        return {
            elapsedMs: elapsed, combatMs: combatMs, died: finalDied,
            deathAtMs: firstDeathAtMs, reviveCount: reviveCount,
            returnBlocked: !!blocker, returnBlocker: blocker,
            potionId: potionId, potionUsed: potionUsed,
            potionBought: potionBought, potionCost: potionCost
        };
    }
    ...
    function _offlineRollMobLoot(mob, kills, map, loot) {
        ...
        try {
            ...
            if ((mob.boss || mob.lv >= 40) && mob.race !== '血盟' && !mob.siegeV2) {
                // 與線上擊殺一致：所有可掉寶頭目（含夢幻之島）固定 0.001%，一般 Lv40+ 敵人維持 0.01%。
                let panRate = mob.boss ? 0.00001 : 0.0001;
                if (panRate > 0) {
            ...
            if (typeof MEM_DROPS !== 'undefined') {
                // 移除舊掉落表中的指定萬能藥，避免少數王在離線期間得到額外機率。
                const panaceaIds = ['panacea_str', 'panacea_dex', 'panacea_con', 'panacea_int', 'panacea_wis', 'panacea_cha'];
                const memDropsWithoutPanacea = Object.create(null);
                Object.keys(MEM_DROPS).forEach(name => {
                    memDropsWithoutPanacea[name] = (MEM_DROPS[name] || []).filter(drop => panaceaIds.indexOf(drop[0]) === -1);
                });
                _offlineRollTable(mob, kills, memDropsWithoutPanacea, dropMult, loot, false);
            }
    ...
    function _offlineGrantBatch(source, profile, elapsed, rawElapsed, efficiency, options) {
        ...
        let survivalPlan = _offlineAutoRevivePlan(profile, elapsed, efficiency);
        ...
        let result = {
            ...
            reviveCount: survivalPlan.reviveCount || 0,
            returnBlocked: survivalPlan.returnBlocked === true,
            returnBlocker: survivalPlan.returnBlocker || null,
        ...
        if (survivalPlan.reviveCount > 0 && !survivalPlan.died && typeof logSys === 'function') {
            logSys('<span class="text-emerald-300 font-bold">離線死亡 ' + survivalPlan.reviveCount + ' 次，已自動復活並回到原練功點繼續狩獵（每次復活等待 30 秒不計收益）。</span>');
        }
        if (survivalPlan.died && survivalPlan.returnBlocked && survivalPlan.returnBlocker && typeof logSys === 'function') {
            logSys('<span class="text-amber-300 font-bold">此地圖需「' + survivalPlan.returnBlocker.name + '」入場；死亡後無法自動回到原練功點，離線收益已停止。</span>');
        }
