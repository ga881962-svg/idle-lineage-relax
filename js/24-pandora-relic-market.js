...
(function () {
    ...
    function _chatContext(w) {
        ...
        return {
            ...
            price: isGold ? _goldBuyerPrice(w).toLocaleString() : '贊助鑽石',
    ...
    function _buyerTitle(w) {
        return _wandererCurrency(w) === 'gold' ? '金幣收購' : '贊助鑽石收購';
    }
    ...
    function wanderingBuyerSpriteData(w) {
        ...
        if (!_classFrameCache[key]) {
            let manifestKey = 'assets/classanim/' + folder;
            let base = (typeof classAnimPath === 'function' ? classAnimPath(folder).slice(0, -1) : manifestKey);
            let manifest = (typeof ANIM_MANIFEST !== 'undefined' && ANIM_MANIFEST) ? ANIM_MANIFEST[manifestKey] : null;
    ...
    function renderWanderingBuyerDialog(div, wandererId) {
        ...
        let rewardText = isGoldBuyer ? `報酬：${goldPrice.toLocaleString()} 金幣` : `報酬：贊助鑽石 × ${Math.max(1, Math.floor(Number(w.reward) || 1))}`;
    ...
    function pandoraRelicSuggestionHTML(query) {
        ...
        return Object.keys(RELIC_CATEGORIES).map(key => {
            ...
            return `<button type="button" class="pandora-buy-suggestion pandora-relic-suggestion" onclick="pandoraChooseRelicSearch('${key}')">${c.label}<span>消耗 ${RELIC_SEARCH_COST} 贊助鑽石</span></button>`;
        }).join('');
    }
    ...
    function pandoraChooseRelicSearch(category) {
        ...
        if (priceEl) {
            priceEl.value = String(RELIC_SEARCH_COST) + ' 贊助鑽石';
    ...
    function pandoraStartRelicSearch(category) {
        ...
        let result = _withStateLock(st => {
            ...
            if (st.diamonds < RELIC_SEARCH_COST) return { commit: false, error: `贊助鑽石不足，需要 ${RELIC_SEARCH_COST} 顆。` };
        ...
        if (!result.ok) {
            ...
            _setPandoraNotice('success', `已在第 ${result.slot + 1} 欄找到${d ? d.n : '遺物'}的布告，消耗 ${RELIC_SEARCH_COST} 顆贊助鑽石。`);
            try { saveGame(); } catch (e) {}
        }
        _rerenderPandora();
    }
    ...
    function pandoraRelicBalanceHTML() {
        let st = _readState();
        return `｜贊助鑽石 <span class="pandora-diamond-count">${st.diamonds.toLocaleString()}</span>`;
    }
    ...
    function pandoraRelicBoardHTML() {
        ...
        return `<section class="pandora-relic-board">
            <div class="pandora-relic-board-head">
                <b>遺物布告欄</b>
                <span>搜尋費用 ${RELIC_SEARCH_COST} 贊助鑽石・完成或取消後，該欄冷卻 24 小時</span>
            </div>
    ...
    function pandoraAdjustSharedDiamonds(delta) {
        ...
        let result = _withStateLock(st => {
            ...
            if (next < 0) return { commit: false, error: '贊助鑽石不足。' };
            st.diamonds = next;
