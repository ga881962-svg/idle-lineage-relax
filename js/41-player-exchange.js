...
(function () {
  ...
  // 僅展示 server listing snapshot，沿用背包同一份素質描述，不提供任何物品操作。
  function itemDetails(item) {
    item = parseItem(item);
    var defs = typeof DB !== 'undefined' && DB && DB.items ? DB.items : ((window.DB && window.DB.items) || {});
    var def = defs[item.id] || {};
    try { if (typeof buildItemDescHTML === 'function') return buildItemDescHTML(item); } catch (_) {}
    return esc(def.d || item.d || '此物品沒有額外素質說明。');
  }
  ...
  function listingCard(row, mine) {
    ...
    return '<article class="px-listing px-has-details" tabindex="0"><div class="px-item-icon">' + itemIcon(item) + '</div><div class="px-item-main"><strong>' + esc(itemName(item)) + ' ×' + quantity + '</strong><span>' + (mine ? '到期：' : '來自 S1・') + esc(left(row.expires_at)) + '</span><button class="px-detail" type="button" onclick="playerExchangeToggleDetails(this)">查看素質</button></div><div class="px-price">💎 ' + money(unit) + '<small>／件・總計 ' + money(total) + '</small></div>' + (mine ? '<button class="px-button px-cancel" onclick="playerExchangeCancel(\'' + esc(row.id) + '\')">取消上架</button>' : '<button class="px-button px-buy" onclick="playerExchangeBuy(\'' + esc(row.id) + '\')">整包購買</button>') + '<section class="px-item-details" aria-label="物品素質"><header>' + esc(itemName(item)) + '</header><div>' + itemDetails(item) + '</div></section></article>';
  }
  async function load() {
  ...
  function createModal() {
    ...
    style.textContent += '.px-listing{position:relative}.px-detail{justify-self:start;border:1px solid #586d9d;border-radius:6px;background:#162442;color:#c6dcff;padding:3px 8px;font-size:12px;font-weight:700;cursor:pointer}.px-detail:hover{background:#223762}.px-item-details{display:none;position:absolute;z-index:5;left:68px;top:62px;width:min(460px,calc(100% - 88px));max-height:min(65vh,420px);overflow:auto;padding:16px;border:1px solid #d1a53a;border-radius:10px;background:#17142a;box-shadow:0 14px 32px rgba(0,0,0,.76);color:#d6deef;font-size:14px;line-height:1.65}.px-item-details header{margin-bottom:10px;padding-bottom:8px;border-bottom:1px solid #58607b;color:#ffe65d;font-size:18px;font-weight:800}.px-has-details:hover .px-item-details,.px-has-details:focus-within .px-item-details,.px-has-details.is-details-open .px-item-details{display:block}@media(max-width:700px){.px-item-details{left:10px;top:58px;width:calc(100% - 20px);max-height:52vh}}';
  ...
  window.playerExchangeToggleDetails = function (button) { var card = button && button.closest('.px-listing'); if (card) card.classList.toggle('is-details-open'); };
