...
const SPECIAL_AREA_BG = {   // 特殊地圖：逐張對應背景
    kent_outer: encodedAreaBg('肯特外門區'), kent_inner: encodedAreaBg('肯特內城'),
    ww_outer: encodedAreaBg('風木外門區'), ww_inner: encodedAreaBg('風木內城'),
    heine_outer: encodedAreaBg('海音外門區'), heine_inner: encodedAreaBg('海音內城'),
    rift_battle: encodedAreaBg('時空裂痕戰場'),
    ...
    dark_elf_sanctuary: encodedAreaBg('黑暗妖精聖地'),
    cursed_dark_elf_sanctuary: encodedAreaBg('受詛咒的黑暗妖精聖地'),
    collapsed_elder_council_hall: encodedAreaBg('崩壞的長老會議廳'),   // 🌑 v3.3.33 長老會議廳改為安全區 town_elder_council（背景走 TOWN_BG_1920）
    ...
    sunrise_castle: encodedAreaBg('日出之國城墎'), sunrise_east: encodedAreaBg('日出之國東之地'),
    sunrise_west: encodedAreaBg('日出之國西之地'), sunrise_north: encodedAreaBg('日出之國北之地'),   // 🌅 日出之國四區（時空裂痕·日本風 1920×1080）
    arena_pvp: encodedAreaBg('決鬥競技場'),   // ⚔️ v3.7.13 決鬥競技場（不在 MAP_CATEGORIES→mapDisplayName 回 null·同名圖走不到，逐張指定完整路徑）
...
// 背景檔原先使用中文檔名；部分本機伺服器會錯誤處理 URL 編碼而回傳 404。
// 統一改用 ASCII 副本（檔名由 encodeURIComponent 的 % 改成 _），保留原圖作素材來源。
function areaBg1920(name) { return (name && AREA_1920.has(name)) ? encodedAreaBg(name) : null; }
function upgradeAreaPath(path) { if (!path) return path; let m = /^assets\/area\/([^\/]+)\.jpg$/.exec(path); return (m && AREA_1920.has(m[1])) ? areaBg1920(m[1]) : path; }
...
async function exportSave(slot){
    ...
    try {
        ...
        alert('匯出失敗：角色、倉庫、寵物、贊助鑽石或血盟資料無法正確讀取，未產生匯出檔。');
        return;
    }
    ...
    input.onchange = function(){
        ...
        reader.onload = function(){
            ...
            if(pandoraDiamonds !== undefined){
                ...
                if(!Number.isFinite(pandoraDiamonds)){ alert('匯入失敗：贊助鑽石資料格式不正確。'); return; }
            ...
            if(pandoraDiamonds !== undefined){
                if(confirm(`此匯入檔包含贊助鑽石 ${pandoraDiamonds.toLocaleString()} 顆。\n是否一併還原贊助鑽石？\n⚠ 會覆蓋目前所有角色共用的贊助鑽石數量。`)){
            ...
            if(restorePandoraDiamonds){
                ...
                if(!_pandoraResult || !_pandoraResult.ok){
                    ...
                    alert('匯入失敗：贊助鑽石資料無法安全還原，角色、倉庫與寵物資料已回復匯入前狀態。');
                    return;
                }
            }
            if(restoreClan){
                ...
                if(!_clanResult || !_clanResult.ok){
                    ...
                    alert('匯入失敗：血盟資料無法安全還原，角色、倉庫、寵物與贊助鑽石資料已嘗試回復匯入前狀態。');
                    return;
                }
            }
            if(restoreWh){
...
function returnToCharacterSelect(){
    ...
    const load = document.getElementById('load-select-panel');
    // 線上存檔不應回到舊版「本機存檔」選角介面；回到登入首頁，
    // 再由首頁的「我的存檔」進入目前帳號的存檔清單。
    const isCloudCharacter = !!(player.cloudCharacterId && typeof window.onlineRosterOpen === 'function');
    if(game) game.classList.add('hidden');
    if(creationScreen) creationScreen.classList.remove('hidden');
    if(main) main.classList.toggle('hidden', !isCloudCharacter);
    if(creation) creation.classList.add('hidden');
    if(load) load.classList.toggle('hidden', isCloudCharacter);
    document.body.classList.remove('game-bg-dim', 'sherine-world', 'sherine-mad');

    if(isCloudCharacter){
        // 清掉遊戲中的選單狀態；保留登入 session，使用者可直接選另一個存檔。
...
function startGame() {
    ...
    player.expMigV = 6;   // 新角色天生使用目前分段休閒經驗曲線，免除進度換算。
...
function loadGame() {
    ...
    if (s) {
        ...
        // 舊存檔／GM 直接調整等級時，補齊 Lv50 起每級應得的能力點。
        // 只補「少掉」的點，已配的點與多出的舊資料都不會被扣除。
        ensureLevelBonusPoints();
        ...
        // v5：Lv1–50 升級需求在 v4 基礎上再減半。既有角色與出戰傭兵維持
        // 當前等級的經驗百分比，避免更新後直接升級或進度看似歸零。
        if ((player.expMigV || 0) < 5) {
            let _mig5 = (lv, exp) => {
                lv = Math.max(1, Math.min(PLAYER_LEVEL_CAP, Math.floor(lv || 1)));
                if (lv >= PLAYER_LEVEL_CAP) return 0;
                let o = getExpReqV4(lv), n = getExpReq(lv);
                if (!isFinite(o) || !isFinite(n) || o <= 0) return 0;
                return Math.min(Math.floor(Math.max(0, exp || 0) / o * n), n - 1);
            };
            player.exp = _mig5(player.lv, player.exp);
            (player.allies || []).forEach(a => { if (a) a.exp = _mig5(a.lv, a.exp); });
            player.expMigV = 5;
        }
        // v6：Lv50–60 升級需求在 v5 基礎上再減半。既有角色與出戰傭兵維持
        // 當前等級的經驗百分比，避免更新後直接升級或進度看似歸零。
        if ((player.expMigV || 0) < 6) {
            let _mig6 = (lv, exp) => {
                lv = Math.max(1, Math.min(PLAYER_LEVEL_CAP, Math.floor(lv || 1)));
                if (lv >= PLAYER_LEVEL_CAP) return 0;
                let o = getExpReqV5(lv), n = getExpReq(lv);
                if (!isFinite(o) || !isFinite(n) || o <= 0) return 0;
                return Math.min(Math.floor(Math.max(0, exp || 0) / o * n), n - 1);
            };
            player.exp = _mig6(player.lv, player.exp);
            (player.allies || []).forEach(a => { if (a) a.exp = _mig6(a.lv, a.exp); });
            player.expMigV = 6;
        }
...
// Lv50 開始，每提升一級獲得 1 點能力點：Lv50=1、Lv99=50。
...
function ensureLevelBonusPoints(p) {
    ...
    // 若 alloc 內有創角重置後併入的點數，它只會讓可補數變少，絕不會多送點。
    let assigned = ['str','dex','con','int','wis','cha'].reduce(function (sum, s) { return sum + p.alloc[s]; }, 0);
    let minimumUnspent = Math.max(0, entitled - assigned);
    let current = Math.max(0, Math.floor(Number(p.bonus) || 0));
    if (current < minimumUnspent) p.bonus = minimumUnspent;
    else p.bonus = current;
    return p.bonus;
}
