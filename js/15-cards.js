...
function _cardMobImg(mob, name) { return typeof safeMonsterIconUrl === 'function' ? safeMonsterIconUrl(mob.img, name) : (mob.img || ('assets/icons/monsters/' + name + '.png')); }
...
function _codexMobThumbHtml(nm, mi, silh) {
    ...
    let enc = typeof monsterAnimPath === 'function' ? monsterAnimPath(nm) : ('assets/anim/' + encodeURIComponent(nm) + '/');
    ...
    let L = '';
    if (hasS) L += `<img src="${enc}${_dp}idle_s_0.png" style="${st}mix-blend-mode:multiply" alt="" aria-hidden="true" onerror="this.style.display='none'">`;
    L += `<img src="${mi.src}" data-fb="${fb}" alt="${nm}" style="${st}" onerror="_mobImgErr(this)">`;
    if (hasW) L += `<img src="${enc}${_dp}idle_w_0.png" style="${st}mix-blend-mode:screen" alt="" aria-hidden="true" onerror="this.style.display='none'">`;
    if (hasW2) L += `<img src="${enc}${_dp}idle_w2_0.png" style="${st}mix-blend-mode:screen" alt="" aria-hidden="true" onerror="this.style.display='none'">`;
