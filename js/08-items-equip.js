...
function doEnhance(targetUid, isEq = true) {
    ...
    //   武器／防具／飾品的超過安定值機率皆由 js/01 的圖鑑曲線統一計算；失敗即爆裝。
    ...
    if (success) {
        let add = (DB.items[scroll.id] && DB.items[scroll.id].isB) ? blessEnhanceGain(target.en) : 1;   // 🌟 祝福卷成功時隨機 +1~+3（不提高成功率）
...
function renderStatusIconBar() {
    ...
    if(bar.dataset.statusSig!==sig){
        ...
        bar.innerHTML=rows.map((x,i)=>{let title=x.label+(x.ticks>0?'｜剩餘 '+x.sec+' 秒':'');let fn=(x.icon||x.name)+'.jpg';return `<div class="status-icon${x.ally?' status-icon--ally':''}" data-status-index="${i}" title="${title}"><img src="assets/state-icons/safe/${encodeURIComponent(fn).replace(/%/g,'_')}" alt="${x.label}"></div>`;}).join('');
    } else {
...
function _updateUIImpl() {
    ...
    let pct = player.lv >= PLAYER_LEVEL_CAP ? 100 : (nxtE > 0 && isFinite(nxtE) ? (player.exp / nxtE) * 100 : 0);
    document.getElementById('txt-exp').innerText = `${pct.toFixed(2)}%`;
    document.getElementById('bar-exp').style.width = `${Math.min(100, pct)}%`;
    // 左側角色卡的即時摘要：全部由目前角色與本圖效率統計計算，不寫入存檔。
    try {
        let _auditNow = (typeof _audit !== 'undefined' && _audit) ? _audit : null;
        let _dpsNowObj = (typeof _dps !== 'undefined' && _dps) ? _dps : null;
        let _mins = Math.max(0.001, (Date.now() - ((_auditNow && _auditNow.start) || Date.now())) / 60000);
        let _goldNow = Math.max(0, player.gold - ((_auditNow && _auditNow.gold0) || player.gold));
        let _dpsNow = Math.round(((_dpsNowObj && _dpsNowObj.player) || 0) / (_mins * 60));
        let _set = (id, value) => { let el = document.getElementById(id); if (el) el.innerText = Number(value || 0).toLocaleString(); };
        ...
        let _pets = (typeof petsOutList === 'function') ? petsOutList().filter(p => p && !p._downed) : [];
        let _petEl = document.getElementById('profile-companion-name');
        if (_petEl) _petEl.innerText = _pets.length ? _pets.map(p => p.name || p.form || '夥伴').join('、') : '未出戰';
        let _weight = Math.max(0, Math.min(100, Number(player.d.weightPct) || 0));
        let _weightFill = document.getElementById('profile-weight-fill'), _weightText = document.getElementById('profile-weight-text');
        if (_weightFill) _weightFill.style.width = _weight + '%';
        if (_weightText) _weightText.innerText = _weight + '%';
