...
// Keep a last-known-good copy.  Clan data is account-wide and must never be
// silently discarded just because a browser storage read temporarily fails.
const CLAN_STATE_BACKUP_KEY = 'fb5_clan_state_v1_bak';
...
function _clanReadStateResult() {
    try {
        ...
        // A previous successful write keeps the preceding state here.  This
        // also makes a missing primary key recoverable without touching saves.
        if (raw == null || raw === '') raw = (typeof _lsGet === 'function') ? _lsGet(CLAN_STATE_BACKUP_KEY) : localStorage.getItem(CLAN_STATE_BACKUP_KEY);
...
function _clanWriteState(st) {
    try {
        ...
        // Save the old, valid payload before replacing it.  It is deliberately
        // raw (not recompressed again), so it can be used immediately on the
        // next page load if the main key goes missing.
        let previous = (typeof _lzGet === 'function') ? _lzGet(CLAN_STATE_KEY) : localStorage.getItem(CLAN_STATE_KEY);
        if (previous != null && previous !== '') {
            if (typeof _lsSet === 'function') _lsSet(CLAN_STATE_BACKUP_KEY, previous);
            else localStorage.setItem(CLAN_STATE_BACKUP_KEY, previous);
        }
        let ok;
...
function clanSyncCurrentPlayer() {
    ...
    if (!info) {
        // Do not erase the role's legacy clan marker here.  The shared clan
        // state can be temporarily absent (storage/server restart), and that
        // marker is the only safe source for the recovery action below.
        return false;
    }
...
// Search current and saved roles for a clan marker from the same game mode.
// This is read-only; restoration is always an explicit player action.
function clanFindLegacyRecovery(p) {
    p = p || player;
    if (!p || !p.cls) return null;
    let targetMode = clanModeKey(p);
    let candidates = [p];
    for (let slot = 1; slot <= 8; slot++) {
        try {
            let raw = (typeof _lzGet === 'function') ? _lzGet('lineage_idle_save_' + slot) : localStorage.getItem('lineage_idle_save_' + slot);
            let unwrapped = typeof _saveUnwrap === 'function' ? _saveUnwrap(raw) : { ok:true, payload:raw };
            if (!unwrapped || !unwrapped.ok || !unwrapped.payload) continue;
            let saved = JSON.parse(unwrapped.payload);
            if (saved && saved.p) candidates.push(saved.p);
        } catch (e) {}
    }
    for (let i = 0; i < candidates.length; i++) {
        let candidate = candidates[i];
        if (!candidate || clanModeKey(candidate) !== targetMode) continue;
        let name = String(candidate.clanName || '').trim().slice(0, 20);
        if (name) return { name:name, faction:candidate.bloodPledge === 'esti' ? 'esti' : 'tros' };
    }
    return null;
}

function clanRestoreLegacyRecovery() {
    if (!player || player.cls !== 'royal') return;
    let recovery = clanFindLegacyRecovery(player);
    if (!recovery) { alert('找不到可修復的舊血盟資料。'); return; }
    let mode = clanModeKey(player), leaderId = clanRoleId(player);
    let result = _clanWithLock(st => {
        if (st.modes[mode]) return { commit:false, error:'此模式已有血盟資料。' };
        st.modes[mode] = { name:recovery.name, leaderId:leaderId, faction:recovery.faction, createdAt:Date.now(), castle:null, guards:null };
        st.members[leaderId] = st.members[leaderId] || { mode:mode, contribution:0, buffOn:false, buffAt:0 };
        return {};
    });
    if (!result.ok) { alert(result.error || '血盟修復失敗。'); return; }
    player.clanName = recovery.name;
    player.bloodPledge = recovery.faction;
    if (typeof saveGame === 'function') saveGame();
...
function clanDonateDiamonds() {
    ...
    let amount = Math.floor(Number(el && el.value) || 0);
    if (amount < 1) { alert('請輸入要捐獻的贊助鑽石數量。'); return; }
    if (typeof window.pandoraAdjustSharedDiamonds !== 'function') { alert('贊助鑽石資料目前無法使用。'); return; }
    let spend = window.pandoraAdjustSharedDiamonds(-amount);
    if (!spend || !spend.ok) { alert((spend && spend.error) || '贊助鑽石不足。'); return; }
    let points = amount * 100;
    let result = _clanAdjustContribution(points);
    if (!result.ok) {
        let refund = window.pandoraAdjustSharedDiamonds(amount);   // 退鑽也可能失敗（共用桶寫入異常）：如實回報，勿讓鑽石默默蒸發
        alert((result.error || '捐獻失敗。') + ((refund && refund.ok) ? '' : ' 且贊助鑽石退回失敗，請重新整理後於黑市確認鑽石數量。'));
        return;
    }
    if (typeof logSys === 'function') logSys(`<span class="text-cyan-300">捐獻 ${amount.toLocaleString()} 顆贊助鑽石，獲得 ${points.toLocaleString()} 貢獻與血盟經驗。</span>`);
    if (typeof updateUI === 'function') updateUI();
    renderClanTab();
}
    ...
    if (!info) {
        ...
        div.innerHTML = `
            <div class="flex flex-col gap-4 p-2">
                ...
                ${recovery && player.cls === 'royal' ? `
                <div class="rounded border border-emerald-600 bg-emerald-950/40 p-3">
                    <div class="font-bold text-emerald-200">找到舊血盟資料：${clanEsc(recovery.name)}</div>
                    <div class="mt-1 text-sm text-slate-300">可直接修復血盟名稱與目前王族身分；不會扣除金幣。</div>
                    <button class="btn mt-3 w-full py-2 font-bold bg-emerald-800 border-emerald-500 text-emerald-100" onclick="clanRestoreLegacyRecovery()">修復血盟</button>
                </div>` : ''}
                ${player.cls === 'royal' ? `
        ...
        <div class="flex flex-col gap-4 p-2">
            ...
            <div class="border-t border-slate-700 pt-3">
                ...
                <div class="flex gap-2">
                    ...
                    <button class="btn px-3 py-2 font-bold bg-cyan-900 border-cyan-600 text-cyan-100" onclick="clanDonateDiamonds()">捐贊助鑽石（持有 ${diamonds.toLocaleString()}）</button>
                </div>
                <div class="text-xs text-slate-400 mt-2">10,000 金幣 = 1 貢獻；1 贊助鑽石 = 100 貢獻。貢獻會增加等量的全模式共用血盟經驗。</div>
            </div>
            <div class="border-t border-slate-700 pt-3">
