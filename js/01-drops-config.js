...
// 圖示素材原本使用中文檔名；改用 safe 資料夾的 ASCII 副本，避開本機伺服器 URL 編碼失敗。
function getSafeIconUrl(folder, filename) {
    return `assets/icons/safe/${folder}/${encodeURIComponent(filename).replace(/%/g, '_')}`;
}
function getIconUrl(d, isSkill = false) {
    if (d.img) {
        const m = /^assets\/icons\/(weapons|armors|accessories|items|skills)\/(.+)$/.exec(d.img);
        return m ? getSafeIconUrl(m[1], m[2]) : d.img;
    }
    
    if (isSkill) return getSafeIconUrl('skills', `${d.n}.png`);
    
    if (d.type === 'wpn') return getSafeIconUrl('weapons', `${d.n}.png`);       // 武器
    if (d.type === 'arm') return getSafeIconUrl('armors', `${d.n}.png`);        // 防具
    if (d.type === 'acc') return getSafeIconUrl('accessories', `${d.n}.png`);   // 飾品
    if (d.type === 'skillbk') return getSafeIconUrl('items', `${d.n}.png`) + '?kind=book';
    
    return getSafeIconUrl('items', `${d.n}.png`);                               // 其他消耗品與道具
}
// 不讓素材載入失敗變成空白格：依物品類型顯示對應備用圖。
// 使用事件捕捉可涵蓋背包、倉庫、商店、圖鑑與 GM 面板既有的所有 <img>。
document.addEventListener('error', function (event) {
    const img = event.target;
    if (!(img instanceof HTMLImageElement) || img.dataset.iconFallbackApplied) return;
    const src = img.getAttribute('src') || '';
    const raw = /^assets\/icons\/(weapons|armors|accessories|items|skills)\/(.+)$/.exec(src);
    // 舊介面仍可能直接指向中文檔名；先自動改走 ASCII 安全路徑，再決定是否顯示備用圖。
    if (raw && !src.includes('assets/icons/safe/') && !img.dataset.iconSafeRetried) {
        img.dataset.iconSafeRetried = '1';
        img.onerror = null;
        img.src = getSafeIconUrl(raw[1], raw[2]);
        return;
    }
    if (!src.includes('assets/icons/safe/')) return;
    const m = /assets\/icons\/safe\/(weapons|armors|accessories|items|skills)\//.exec(src);
    let fallback = 'item-fallback.png';
    if (src.includes('kind=book')) fallback = 'book-fallback.png';
    else if (m) fallback = ({ weapons:'weapon-fallback.png', armors:'armor-fallback.png', accessories:'accessory-fallback.png', items:'item-fallback.png', skills:'book-fallback.png' }[m[1]] || fallback);
    img.dataset.iconFallbackApplied = '1';
    img.onerror = null;
    img.style.opacity = '1';
    img.style.display = '';
    img.src = 'assets/ui/' + fallback;
    // 有些舊元件在自己的 onerror 內把圖設為透明；下一個畫面格強制還原，避免備用圖又被隱藏。
    requestAnimationFrame(function () { img.style.opacity = '1'; img.style.display = ''; });
}, true);
...
// 🩸 所有怪物（一般怪、菁英、頭目）預設固定顯示血條。
// 舊存檔曾記住「關閉」時也強制改為開啟，避免玩家看不到怪物剩餘血量。
let _showMobHp = true;
(function(){ try { localStorage.setItem('lineage_idle_mob_hp', '1'); } catch(e){} })();
...
//   武器：+0~+5 35%、+6 60%、+7 50%、+8 40%、+9~+19 35%（未達安定值仍必成）
//   防具：安全值0/4為 +0~+4 50%、+5 40%、+6 30%、+7~+14 20%；安全值6為 +6 30%、+7~+14 20%
//   飾品：+0 50%、+1 40%、+2 30%、+3~+4 20%；失敗一律爆裝
function enhanceOutcomeFromRoll(type, safe, en, r) {
    // Same curve as the referenced enchant guide.  The safe threshold is the
    // only guaranteed area; every failed roll beyond it destroys the item.
    if (en < safe) return 'ok';
    let rate;
    if (type === 'wpn') {
        rate = en <= 5 ? 0.35 : (en === 6 ? 0.60 : (en === 7 ? 0.50 : (en === 8 ? 0.40 : 0.35)));
    } else if (type === 'acc') {
        rate = en === 0 ? 0.50 : (en === 1 ? 0.40 : (en === 2 ? 0.30 : 0.20));
    } else if (safe === 0 || safe === 4) {
...
function enhanceRollOutcome(d, en) {   // 擲一次衝裝骰 → 'ok'成功 | 'break'爆裝
    let safe = (d && d.safe) || 0;
    en = Number(en) || 0;
    let r = en < safe ? 0 : Math.random();   // 🎲 安定值內不消耗亂數；其餘每次嘗試獨立，可 save/load 重抽
    return enhanceOutcomeFromRoll(d && d.type, safe, en, r);
}
// 🌟 祝福卷軸成功時的提升量：每一次成功皆等機率 +1／+2／+3（不提高成功率）。
function blessEnhanceGainFromRoll(en, r) { return 1 + Math.floor(r * 3); }
function blessEnhanceGain(en) {
    en = Number(en) || 0;
    return blessEnhanceGainFromRoll(en, Math.random());
}
...
//  額外傷害：+0~+20 每階+1（最高 +20）；
//  額外命中：+0~+10 每階+1，+10 之後依 WPN_EN_HIT_OVER10 累加。
const WPN_EN_HIT_OVER10 = { 11:1, 12:2, 13:4, 14:6, 15:8, 16:11, 17:14, 18:17, 19:21, 20:25 };   // +20 時累計命中 +35
...
// 防具強化 → AC 減免量：+1~+10 每階 AC-1，+11~+15 每階 AC-2（+15 共 AC-20）。
function enhanceArmAc(en) {
    en = Math.min(Math.max(0, Number(en) || 0), ENHANCE_CAP.arm);
    return en <= 10 ? en : 10 + (en - 10) * 2;
}
