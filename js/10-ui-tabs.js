...
function renderTabs(force) {
    ...
    // 裝備清單固定顯示目前負重，與每個部位的列式資訊一起閱讀。
    let _equipWeight = document.createElement('div');
    _equipWeight.className = 'equip-list-weight';
    _equipWeight.innerHTML = `負重 <b>${Math.max(0, Math.round(Number(player.d && player.d.weightPct) || 0))}%</b>`;
    eDiv.appendChild(_equipWeight);
...
player.inv.forEach(i => {
    ...
    // 裝備頁使用直式配戴清單，不套用背包的格狀外框與空白格。
    [wDiv,aDiv,iDiv].forEach(decorateClassicInventoryTab);
...
function buildItemDescHTML(item) {
    ...
    if(d.type === 'wpn') {
        ...
        // 顯示這把「實例武器」的強化最終傷害倍率；倍率由戰鬥端同一個規則函式計算。
        const _tipWpnEn = (typeof capWpnEn === 'function') ? capWpnEn(item && item.en) : Math.max(0, Number(item && item.en) || 0);
        const _tipFinalMult = (typeof enhanceWpnFinalMult === 'function') ? enhanceWpnFinalMult(_tipWpnEn, d) : 1;
        if (_tipWpnEn > 0) {
            const _tipFinalPct = Math.round((_tipFinalMult - 1) * 10000) / 100;
            desc += `<br><span class="text-amber-300 font-bold">武器強化 +${_tipWpnEn}：最終傷害倍率 ×${Number(_tipFinalMult).toFixed(2)}（+${_tipFinalPct}%）</span>`;
        }
    ...
    if(d.type === 'arm' || d.type === 'acc') {
        ...
        // 裝備說明必須顯示「強化後」真正帶來的防禦效果，避免玩家只看到基礎 AC。
        const _tipEn = Math.max(0, Number(item && item.en) || 0);
        if (d.type === 'arm' && !d.armguard && _tipEn > 0) {
            const _tipExtraAc = (typeof enhanceArmAc === 'function') ? enhanceArmAc(_tipEn) : _tipEn;
            const _tipTotalAc = (Number(d.ac) || 0) + _tipExtraAc;
            desc += `<br><span class="text-cyan-300 font-bold">防具強化 +${_tipEn}：額外 AC -${_tipExtraAc}（總 AC ${_tipTotalAc >= 0 ? '-' + _tipTotalAc : '+' + (-_tipTotalAc)}）</span>`;
        }
        // 戒指強化每 +1 額外提供 AC -1（最多採計 +5）；其餘飾品的強化效果由 MR 等屬性顯示。
        if (d.type === 'acc' && d.slot === 'ring' && _tipEn > 0) {
            const _tipRingAc = Math.min(_tipEn, 5);
            const _tipTotalAc = (Number(d.ac) || 0) + _tipRingAc;
            desc += `<br><span class="text-cyan-300 font-bold">戒指強化 +${_tipEn}：額外 AC -${_tipRingAc}（總 AC ${_tipTotalAc >= 0 ? '-' + _tipTotalAc : '+' + (-_tipTotalAc)}）</span>`;
        }
        let isRanged = (d.ranged === true);
        let hitLabel = isRanged ? "遠距離命中" : "近距離命中";
        let dmgLabel = isRanged ? "遠距離傷害" : "近距離傷害";
    ...
    if (d.type === 'wpn' || d.type === 'arm' || d.type === 'acc') {
        ...
        let _logos = _EQ_CLASSES
            ...
            .map(([, nm]) => `<img src="${typeof assetUrl === 'function' ? assetUrl('assets/logo/' + nm + 'icon.png') : ('assets/logo/' + nm + 'icon.png')}" alt="${nm}" title="${nm}" class="class-eq-icon" onerror="this.style.display='none';">`)
            .join('');
...
function _quickEnhanceUnit(d, startEn, goal, scrollStacks, useBless) {
    ...
    while (en < goal) {
        ...
        let add = bless ? blessEnhanceGain(en) : 1;   // 🌟 祝福卷成功時隨機 +1~+3；一般卷 +1
...
function buildQuickEnhanceHeader(type) {
    ...
    let opts = '';
    for (let t = 1; t <= enhanceCap({ type }); t++) opts += `<option value="${t}" ${t === target ? 'selected' : ''}>+${t}</option>`;
    ...
    let _blessUi = type === 'acc' ? '' : `<label class="flex items-center gap-1 text-xs ${_blessCnt > 0 ? 'text-yellow-300' : 'text-slate-500'} cursor-pointer select-none whitespace-nowrap" title="勾選＝使用『祝福的卷軸』強化（成功時隨機 +1~+3）；不勾＝一般卷軸（+1）。"><input type="checkbox" ${st.useBless ? 'checked' : ''} onchange="quickEnh['${type}'].useBless=this.checked"> 祝福卷(${_blessCnt})</label>`;
...
function runQuickEnhance(type) {
    ...
    entries.forEach(entry => {
        ...
        for (let u = 0; u < cnt; u++) {
            ...
            usedTotal += r.used;
            let _qeName = getItemFullName({ ...entry, en: r.en });
            if (r.destroyed) logSys(`<span class="text-red-400 font-bold">快速強化失敗：</span>${_qeName} 強化失敗，裝備損壞。`);
            else if (r.en >= goal) logSys(`<span class="text-emerald-300 font-bold">快速強化成功：</span>${_qeName} 已達目標 +${goal}。`);
            else logSys(`<span class="text-amber-300 font-bold">快速強化未完成：</span>${_qeName} 停在 +${r.en}（卷軸不足或強化未成功）。`);
...
function buildQuickHeader(type) {
    ...
    btns += `<button onclick="autoSellJunk(true)" class="flex-1 btn border-emerald-700 bg-emerald-900/70 hover:bg-emerald-800 py-1.5 text-sm font-bold text-emerald-100 rounded shadow" title="立即賣出所有已標記的廢品">💰 賣出廢品</button>`;
...
function renderLegacyPvpTab() {
    ...
    let wildNpcEnabled = typeof WILD_PLAYER_NPC_ENABLED === 'undefined' || !!WILD_PLAYER_NPC_ENABLED;
    if (warForced && wildNpcEnabled) player.pvpOn = true;
    let pvpOn = wildNpcEnabled && !!player.pvpOn;
    ...
    let pvpHint = '野外假玩家已停用；血盟與野外對戰規則正在重製。競技場功能不受影響。';
        ...
        <div class="flex flex-col gap-3">
            ...
            <div class="${pvpBoxCls} border rounded p-3">
                <label class="flex items-center justify-between gap-3 cursor-pointer">
                    ...
                    <input type="checkbox" class="w-5 h-5 accent-red-600" ${pvpOn ? 'checked' : ''} ${(warForced || !wildNpcEnabled) ? 'disabled' : ''} onchange="setPvpMode(this.checked)">
                </label>
...
function setPvpMode(on) {
    if (typeof WILD_PLAYER_NPC_ENABLED !== 'undefined' && !WILD_PLAYER_NPC_ENABLED) {
        player.pvpOn = false;
        renderPvpTab();
        return;
    }
...
function switchTab(t, btn) {
    ...
    // 只有「能力」允許顯示能力面板；其他所有分頁皆為各自的獨立內容。
    if (_tabHost) _tabHost.classList.toggle('inventory-only', t !== 'stats');
    // 裝備改由分頁內的直式清單呈現；不再覆蓋成舊式紙娃娃視窗。
